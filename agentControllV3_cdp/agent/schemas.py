...
def validate_action(raw: Any) -> Tuple[bool, str, Dict[str, Any]]:
    ...
    elif name in ("click_element", "type_in_element"):
        eid = raw.get("element_id")
        if not _is_int(eid):
            return False, f"{name} 需要整數 element_id", {}
        action["element_id"] = int(eid)
        if name == "type_in_element":
            text = raw.get("text")
            if not isinstance(text, str):
                return False, "type_in_element 需要字串 text", {}
            action["text"] = text
