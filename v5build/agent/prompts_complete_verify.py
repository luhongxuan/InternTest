...
你看到一張截圖，判斷「目前這個 plan step 是否已經完成」。
你不執行任何動作，只做判斷。
只輸出 JSON，不要多餘文字。"""

VERIFY_USER_TEMPLATE = """任務：{task}
...
規則：
- 如果 step 目標已達成 → action = "advance_step"
- 如果 step 尚未完成，但可以繼續 → action = "continue"
- 如果畫面狀態明顯錯誤無法繼續 → action = "task_failed"
- 只輸出 JSON。"""
...
def build_verify_user(
    ...
    return VERIFY_USER_TEMPLATE.format(
        ...
        success_checks=step_info.get("success_checks", "（未定義）"),
        history=history_text or "(尚無)",
        page_url=page_url or "(未知)",
        page_title=page_title or "(未知)",
