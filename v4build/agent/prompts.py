...
原有 Planning prompt 不動。
Execution prompt 新增 browser 版本（元素清單 + 截圖 + 通用 action schema）。
保留原有 screenshot-only 版本作為 fallback。
"""
...
你每一步會收到：目前頁面的可互動元素清單，以及一張頁面截圖。
請根據任務目標與當前畫面，決定「下一個」動作。每次只輸出一個 JSON 動作，不要多餘文字。

可用動作（只能用這些）：
  click_element      {"action":"click_element","element_id":"xxx","reason":""}
  click_coordinate   {"action":"click_coordinate","x":0,"y":0,"reason":""}  ← 找不到結構元素時的 fallback
  type_text          {"action":"type_text","text":"...","reason":""}
  press_key          {"action":"press_key","key":"Enter","reason":""}
  hotkey             {"action":"hotkey","keys":["Ctrl","L"],"reason":""}
  scroll             {"action":"scroll","amount":-300,"reason":""}  ← 正=上，負=下
...
# ---------------------------------------------------------------------------
# Execution — 原有 Screenshot-only 版（fallback，不動）
# ---------------------------------------------------------------------------
...
可用動作（只能用這些）：
click / double_click / right_click / move_mouse：需要 x 還有 y（以你看到的這張圖的像素為準）
